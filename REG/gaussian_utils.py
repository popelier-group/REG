...
def get_control_coordinates_PES_Scan(g16_output_filelist, atom_list):
    ...
    for i in range(0, len(xyz_files)):
        ...
        coordinates1_list = f1.readlines()[
            2:
        ]  # temporary remove the first 2 lines of xyz file
        atom1 = [
            float(c)
            for c in re.findall(r"[-+]?\d*\.\d+|\d+", coordinates1_list[atom_list[0]])
        ]
        atom2 = [
            float(c)
            for c in re.findall(r"[-+]?\d*\.\d+|\d+", coordinates1_list[atom_list[1]])
        ]
        x = 0
        y = 1
        z = 2
