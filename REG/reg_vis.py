...
def generate_data_vis(
    ...
    n_term,
    save=False,
    file_name="data_vis.png",
    title="REG data visualization",
):
    (mu, sigma) = norm.fit(file["R"])
    fig = plt.figure(constrained_layout=False, figsize=[10, 8])
    fig.suptitle(title)
    gs1 = fig.add_gridspec(
        nrows=6, ncols=6, top=0.9, bottom=0.65, hspace=0, wspace=0.40
    )
    ...
    ax2.hist(
        file["R"],
        bins=[0.1 * i for i in range(-11, 11, 1)],
        density=0,
        color="green",
        edgecolor="black",
        linewidth=1.2,
    ...
    text_p = [
        ax4.text(n["R"][i], n["REG"][i], n["TERM"][i], bbox=bbox_props)
...
def plot_segment(
    ...
    graph.xaxis.set_major_locator(
        MultipleLocator(2 * abs(coordinate[0] - coordinate[1]))
    )
    ...
    graph.axvline(
        coordinate[0], linestyle="dotted", color="black"
    )  # the first segment starts at the global minimum.
    ...
    if color == True:  # Color_segments
        graph.axvspan(
            graph.get_xlim()[0],
            graph.get_xlim()[1],
            facecolor=color_list[len(critical_points)],
            alpha=0.2,
        )
        start = 0
...
def pandas_REG_dataframe_to_table(dataframe, table_name, SAVE_FIG=True):
    if SAVE_FIG == True:
        ...
        t = ax.table(
            ...
            colWidths=[0.4] * len(dataframe.columns),
...
def filter_term_dataframe(prop_dataframe, original_prop_name, new_prop_name):
    ...
    temp = re.escape(original_prop_name)
    new_df = prop_dataframe[prop_dataframe["TERM"].str.contains(temp)]
    new_df = new_df[col]
    new_df = new_df.sort_values("REG").reset_index(drop=True)
    new_df["TERM"] = new_df["TERM"].apply(
        lambda row: row.replace(original_prop_name + "-", new_prop_name + "(")
    )
