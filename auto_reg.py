...
inter_prop = [
    ...
    "VX_IQA(A,B)",
    "E_IQA_Inter(A,B)",
]  # chose the AIMAll inter atomic properties to analyse
inter_prop_names = [
    "Vcl",
    "Vxc",
    "Einter",
...
### DISPERSION OPTIONS ###
DFT_D3_PATH = (
    "/mnt/pp01-home01/v69787ff/SOFTWARE/DFT-D3/dftd3"  # insert path of DFT-D3 program
)
...
n_terms = 10  # number of terms to rank in figures and tables
...
try:
    ...
    print(
        "Creation of the directory {a}/{b}_results failed or has already been created".format(
            a=cwd, b=SYS
        )
    )
else:
...
atomic_files = [
    cwd + "/" + reg_folders[i] + "/" + wfn_filelist[i][:-4] + "_atomicfiles"
    for i in range(0, len(reg_folders))
]
g16_files = [
    cwd + "/" + reg_folders[i] + "/" + g16_out_file[i]
    for i in range(0, len(reg_folders))
]
...
iqa_intra, iqa_intra_header = aim_u.intra_property_from_int_file(
    atomic_files, intra_prop, atoms
)
iqa_intra_header = np.array(iqa_intra_header)  # used for reference
iqa_intra = np.array(iqa_intra)
# GET INTER-ATOMIC TERMS:
iqa_inter, iqa_inter_header = aim_u.inter_property_from_int_file(
    atomic_files, inter_prop, atoms
)
...
if CHARGE_TRANSFER_POLARISATION:
    (
        iqa_charge_transfer_terms,
        iqa_charge_transfer_headers,
        iqa_polarisation_terms,
        iqa_polarisation_headers,
    ) = aim_u.charge_transfer_and_polarisation_from_int_file(
...
if DISPERSION:
    rmse_integration = reg.integration_error(
        total_energy_wfn, total_energy_iqa + total_energy_dispersion
    )
else:
...
if WRITE:
    ...
    writer = pd.ExcelWriter(
        path=cwd + "/" + SYS + "_results/REG.xlsx", engine="xlsxwriter"
    )
    energy_writer = pd.ExcelWriter(
        path=cwd + "/" + SYS + "_results/Energy.xlsx", engine="xlsxwriter"
    )
    ...
    for i in range(len(reg_inter[0])):
        ...
        for j in range(len(inter_prop)):
            df_property = rv.filter_term_dataframe(
                df_inter, inter_prop[j], inter_prop_names[j]
            )
            if j <= 1:
                properties_comparison.append(df_property)
            df_property.to_csv(
                inter_prop_names[j] + "_seg_" + str(i + 1) + ".csv", sep=","
            )
            df_property.to_excel(
                writer, sheet_name=inter_prop_names[j] + "_seg_" + str(i + 1)
            )
            list_property_sorted.append(
                pd.concat(
                    [df_property[-n_terms:], df_property[:n_terms]], axis=0
                ).sort_values("REG")
            )
        for j in range(len(intra_prop)):
            df_property = rv.filter_term_dataframe(
                df_intra, intra_prop[j], intra_prop_names[j]
            )
            if j == 0:
                properties_comparison.append(df_property)
            df_property.to_csv(
                intra_prop_names[j] + "_seg_" + str(i + 1) + ".csv", sep=","
            )
            df_property.to_excel(
                writer, sheet_name=intra_prop_names[j] + "_seg_" + str(i + 1)
            )
            list_property_sorted.append(
                pd.concat([df_property], axis=0).sort_values("REG")
            )
    ...
    if DISPERSION:
        ...
        for i in range(len(reg_inter[0])):
            ...
            df_disp_new.dropna(
                axis=0, how="any", subset=None, inplace=True
            )  # get rid of "NaN" terms which have a null REG Value
        ...
        ).to_excel(energy_writer, sheet_name="dispersion_energies")
    # CHARGE-TRANSFER and POLARISATION
    if CHARGE_TRANSFER_POLARISATION:
        df_ct_pl_sorted = pd.DataFrame()
        for i in range(len(reg_inter[0])):
            df_pl = rv.filter_term_dataframe(
                rv.create_term_dataframe(reg_pl, iqa_polarisation_headers, i),
                "Vpl_IQA(A,B)",
                "Vpl",
            )
            df_ct = rv.filter_term_dataframe(
                rv.create_term_dataframe(reg_ct, iqa_charge_transfer_headers, i),
                "Vct_IQA(A,B)",
                "Vct",
            )
            ...
            df_temp = (
                pd.concat([df_pl, df_ct]).sort_values("REG").reset_index(drop=True)
            )
            df_ct_pl_sorted = pd.concat(
                [
                    df_ct_pl_sorted.reset_index(drop=True),
                    pd.concat([df_temp[-n_terms:], df_temp[:n_terms]], axis=0)
                    .sort_values("REG")
                    .reset_index(drop=True),
                ],
                axis=1,
        ...
        rv.pandas_REG_dataframe_to_table(
            df_ct_pl_sorted, "REG_Vct-Vpl_table", SAVE_FIG=SAVE_FIG
        )
        pd.DataFrame(
            data=np.concatenate(
    ...
    for i in range(len(reg_inter[0])):
        ...
        df_final_sorted = pd.concat(
            [
                df_final_sorted.reset_index(drop=True),
                pd.concat([df_final[-n_terms:], df_final[:n_terms]], axis=0)
                .sort_values("REG")
                .reset_index(drop=True),
            ],
            axis=1,
...
rv.plot_segment(
    ...
    annotate=ANNOTATE,
    label=LABELS,
    y_label=r"Relative Energy [$kJ.mol^{-1}$]",
    x_label=X_LABEL,
    title=SYS,
    save=SAVE_FIG,
