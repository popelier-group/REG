...
def find_critical(Y,X, min_points=3, use_inflex=False):
    ...
    if use_inflex == True:        
        ...
        for i in range(1, len(Y_prime)-1):
            ...
            if Y_prime[i] < 0 and Y_prime[i+1] > 0:
                critical_point.append(i+1)
            if Y_prime[i-1] < Y_prime[i] and Y_prime[i] > Y_prime[i+1]:
                critical_point.append(i+1)
            if Y_prime[i-1] > Y_prime[i] and Y_prime[i] < Y_prime[i+1]:
                critical_point.append(i+1)
    ...
    for i in range(1, len(critical_point)-1):
        if critical_point[i] - critical_point_list[-1] > min_points:
...
def reg(wfn_energy, control_coord, terms, critical=True, np=5, inflex=False, critical_index= [ ], mode="norm"):
    #ERRORS:
