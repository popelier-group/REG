# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 15:14:50 2021
...
def REGNBO(SYS0, path1, F1, R1, path2, F2, R2, inv_1, inv_2, groups, POINTS, INFLEX, WRITE, AUTO, AUTO_fit, FORCE, COLOR_TERMS, COLOR_SIGN, SIZE_TITLE,
    ...
    for i in range(len(out_files1)):
        ...
        """
        for j in range(len(out_files1[i])):
            logtemp.append(out_files1[i][j].replace('.out','_NBO7.log'))
        """
    #print(log_files1)
    ...
    if NOFIT == True:
        ...
        indice_removal=[]
        for i in range(len(reg_nbo1[0])):
            for j in range(len(reg_nbo1[0][i])):
                    if reg_nbo1[0][i][j] == 0:
                        print("NBO_1",i,j,reg_nbo1[0][i][j],nbo_int1[i][j],nbo_header1[i][j])
                        indice_removal.append([i,j])
        for i in range(len(indice_removal)):
            reg_nbo1[0][indice_removal[i][0]].pop(indice_removal[i][1])
            reg_nbo1[1][indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_header1[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_int1[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_diff1[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_F1[indice_removal[i][0]].pop(indice_removal[i][1])
        indice_removal=[]
        for i in range(len(reg_nbo2[0])):
            for j in range(len(reg_nbo2[0][i])):
                    if reg_nbo2[0][i][j] == 0:
                        print("NBO_2",i,j,reg_nbo2[0][i][j],nbo_int2[i][j],nbo_header2[i][j])
                        indice_removal.append([i,j])
        for i in range(len(indice_removal)):
            reg_nbo2[0][indice_removal[i][0]].pop(indice_removal[i][1])
            reg_nbo2[1][indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_header2[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_int2[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_diff2[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_F2[indice_removal[i][0]].pop(indice_removal[i][1])
    """   
    ...
    if SYS0 == "BCl2":
        crit_list_diff.pop(2)
        crit_list_diff.pop(2)
        crit_list_diff.pop(2)
        crit_list_diff.pop(5)
        crit_list_diff.pop(5)
        crit_list_diff.pop(5)
        crit_list_diff.pop(5)
    if SYS == "BH2":
        crit_list_diff.pop(6)
        crit_list_diff.pop(3)
        crit_list_diff.pop(2)
        crit_list_diff.pop(0)
    ...
    crd = reg.norm_diff(cc_a1, cc_a2, cc_a_sampled, crit_list_diff, crit_list1_0N, crit_list2_0N, verbose=verbose) #for BCl2
    crd1=crd[0]
    crd2=crd[1]
    if verbose==True:
        print("crd1",crd[0])
    ...
    if verbose==True:
        print("check",len(total_energy_wfx),len(cc_a_sampled),len(nbo_intE2))
        for i in range(len(nbo_intE2)):
            print(len(nbo_intE2[i]))
    ...
    if verbose==True:
        print("Term signs for E2 terms, path 1",sign_seg_E21)
    ...
    sign_seg2 = [sign_seg_E22,sign_seg_E42] #not really useful since terms can't change sign and are the same on both pathways
    ...
    sign_seg = [sign_seg_intra,sign_seg_inter]
    #print(sign_seg)
    """
    """
    if SYS0 == "BH2":
