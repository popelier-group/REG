# -*- coding: utf-8 -*-
"""
Created on Wed Jun  9 12:45:28 2021
...
def REGNBO(SYS0, path1, F1, R1, path2, F2, R2, inv_1, inv_2, groups, POINTS, INFLEX, WRITE, AUTO, AUTO_fit, FORCE, COLOR_TERMS, COLOR_SIGN, SIZE_TITLE,
    ...
    log_files1 = []
    for i in range(len(out_files1)):
        log_files1.append(out_files1[i].replace('.out','_NBO7.log'))
    ...
    log_files2 = []
    for i in range(len(out_files2)):
        log_files2.append(out_files2[i].replace('.out','_NBO7.log'))
    ...
    nbo_prop1 = nbo_utils.nbo7_property_from_g16_file(log_files1, verbose=verbose)
    nbo_prop2 = nbo_utils.nbo7_property_from_g16_file(log_files2, verbose=verbose)
    #print(nbo_prop1[0][0])
    #print(nbo_prop1[1][0])
    if SYS0 == "BH2" and verbose==True:
        print("Indices of term: 10.BD(1)C2-C3/19.LV(1)B10")
        print("Path 1, segment 1",nbo_prop1[0][0].index('10.BD(1)C2-C3/19.LV(1)B10'))
        print("Path 1, segment 2",nbo_prop1[0][1].index('10.BD(1)C2-C3/19.LV(1)B10'))
        print("Path 2, segment 1",nbo_prop2[0][0].index('10.BD(1)C2-C3/19.LV(1)B10'))
        print("Path 2, segment 2",nbo_prop2[0][1].index('10.BD(1)C2-C3/19.LV(1)B10'))
    if SYS0 == "NH2" and verbose==True:
        print("Indices of term: 6.LP(1)N10/24.BD*(1)C2-C3")
        print("Path 1, segment 1",nbo_prop1[0][0].index('6.LP(1)N10/24.BD*(1)C2-C3')) 
        print("Path 1, segment 2",nbo_prop1[0][1].index('6.LP(1)N10/24.BD*(1)C2-C3'))
        print("Path 2, segment 1",nbo_prop2[0][0].index('6.LP(1)N10/24.BD*(1)C2-C3'))
        print("Path 2, segment 2",nbo_prop2[0][1].index('6.LP(1)N10/24.BD*(1)C2-C3'))        
    ...
    nbo_trim1 = nbo_utils.nbo_trim_nbo7(nbo_prop1, crit_list1_0N, verbose=verbose)
    nbo_headerE2_1 = nbo_trim1[0]
    nbo_intE2_1 = nbo_trim1[1]
    nbo_headerE4_1 = nbo_trim1[4]
    nbo_intE4_1 = nbo_trim1[6]
    
    if SYS0 == 'BH2' and verbose==True:
        print("Indices of term: 10.BD(1)C2-C3/19.LV(1)B10")
        print("Path 1, segment 1",nbo_intE2_1[0][nbo_headerE2_1[0].index('10.BD(1)C2-C3/19.LV(1)B10')])
    if SYS0 == 'NH2' and verbose==True:
        print("Indices of term: 6.LP(1)N10/24.BD*(1)C2-C3")
        print("Path 1, segment 1",nbo_intE2_1[0][nbo_headerE2_1[0].index('6.LP(1)N10/24.BD*(1)C2-C3')])
    nbo_diff1 = nbo_trim1[2]
    nbo_F1 = nbo_trim1[3]
    nbo_intSE4_1 = nbo_trim1[5]
    
    nbo_trim2 = nbo_utils.nbo_trim_nbo7(nbo_prop2, crit_list2_0N, verbose=verbose) 
    nbo_headerE2_2 = nbo_trim2[0]
    nbo_intE2_2 = nbo_trim2[1]
    nbo_headerE4_2 = nbo_trim2[4]
    nbo_intE4_2 = nbo_trim2[6]
    if verbose==True:
        print("t1",len(nbo_intE4_1),len(nbo_intE4_1[0]))
        print("t2",len(nbo_intE4_2),len(nbo_intE4_2[0]))
        print("t1",len(nbo_headerE4_1),len(nbo_headerE4_1[0]))
        print("t2",len(nbo_headerE4_2),len(nbo_headerE4_2[0]))
        
        for i in range(len(nbo_headerE4_1)):
            print("E4_1",i,len(nbo_headerE4_1[i]))
        for i in range(len(nbo_headerE4_2)):
            print("E4_2",i,len(nbo_headerE4_2[i]))
    
    if SYS0 == 'BH2' and verbose==True:
        print("Indices of term: 10.BD(1)C2-C3/19.LV(1)B10")
        print("Path 2, segment 1",nbo_intE2_2[0][nbo_headerE2_2[0].index('10.BD(1)C2-C3/19.LV(1)B10')])
    if SYS0 == 'NH2' and verbose==True:
        print("Indices of term: 6.LP(1)N10/24.BD*(1)C2-C3")
        print("Path 2, segment 1",nbo_intE2_2[0][nbo_headerE2_2[0].index('6.LP(1)N10/24.BD*(1)C2-C3')])
    nbo_diff2 = nbo_trim2[2]
    nbo_F2 = nbo_trim2[3]
    nbo_intSE4_2 = nbo_trim2[5]
    ...
    """
        ...
        i+=1
    """
    for i in range(len(nbo_headerE2_1)):
        for j in range(len(nbo_headerE2_1[i])):
            for x in nbo_headerE2_1[i][j]:#[k]:
                if x == "/" :
                    rep=nbo_headerE2_1[i][j].split("/") 
                    nbo_headerE2_1[i][j] = rep[0] + "\n" + rep[1]
    for i in range(len(nbo_headerE2_2)):
        for j in range(len(nbo_headerE2_2[i])):
            for x in nbo_headerE2_2[i][j]:
                if x == "/" :
                    rep=nbo_headerE2_2[i][j].split("/") 
                    nbo_headerE2_2[i][j] = rep[0] + "\n" + rep[1]
    for i in range(len(nbo_headerE4_1)):
        for j in range(len(nbo_headerE4_1[i])):
            for x in nbo_headerE4_1[i][j]:#[k]:
                if x == "/" :
                    rep=nbo_headerE4_1[i][j].split("/") 
                    nbo_headerE4_1[i][j] = rep[0] + "\n" + rep[1]
    for i in range(len(nbo_headerE4_2)):
        for j in range(len(nbo_headerE4_2[i])):
            for x in nbo_headerE4_2[i][j]:
                if x == "/" :
                    rep=nbo_headerE4_2[i][j].split("/") 
                    nbo_headerE4_2[i][j] = rep[0] + "\n" + rep[1]
    ...
    if NOFIT == True:
        reg_nboE2_1 = reg.regnbo(total_energy_wfx1, cc_a1, nbo_intE2_1, np=POINTS,
                          critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list1, verbose=verbose)
        reg_nboE4_1 = reg.regnbo(total_energy_wfx1, cc_a1, nbo_intE4_1, np=POINTS,
                          critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list1, verbose=verbose)
        reg_nboE2_2 = reg.regnbo(total_energy_wfx2, cc_a2, nbo_intE2_2, np=POINTS,
                          critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list2, verbose=verbose)
        reg_nboE4_2 = reg.regnbo(total_energy_wfx2, cc_a2, nbo_intE4_2, np=POINTS,
                          critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list2, verbose=verbose)
        
        reg_trimE2_1    = reg.reg_zeroes(reg_nboE2_1, [nbo_headerE2_1, nbo_intE2_1, nbo_diff1, nbo_F1], verbose=verbose)
        reg_nboE2_1     = reg_trimE2_1[0]
        nbo_headerE2_1  = reg_trimE2_1[1][0]
        nbo_intE2_1     = reg_trimE2_1[1][1]
        nbo_diff1       = reg_trimE2_1[1][2]
        nbo_F1          = reg_trimE2_1[1][3]
        
        reg_trimE4_1    = reg.reg_zeroes(reg_nboE4_1, [nbo_headerE4_1, nbo_intE4_1, nbo_intSE4_1], verbose=verbose)
        reg_nboE4_1     = reg_trimE4_1[0]
        nbo_headerE4_1  = reg_trimE4_1[1][0]
        nbo_intE4_1     = reg_trimE4_1[1][1]
        nbo_intSE4_1    = reg_trimE2_1[1][2]
        
        reg_trimE2_2    = reg.reg_zeroes(reg_nboE2_2, [nbo_headerE2_2, nbo_intE2_2, nbo_diff2, nbo_F2], verbose=verbose)
        reg_nboE2_2     = reg_trimE2_2[0]
        nbo_headerE2_2  = reg_trimE2_2[1][0]
        nbo_intE2_2     = reg_trimE2_2[1][1]
        nbo_diff2       = reg_trimE2_2[1][2]
        nbo_F2          = reg_trimE2_2[1][3]
        
        reg_trimE4_2    = reg.reg_zeroes(reg_nboE4_2, [nbo_headerE4_2, nbo_intE4_2, nbo_intSE4_2], verbose=verbose)
        reg_nboE4_2     = reg_trimE4_2[0]
        nbo_headerE4_2  = reg_trimE4_2[1][0]
        nbo_intE4_2     = reg_trimE4_2[1][1]
        nbo_intSE4_2    = reg_trimE2_2[1][2]
    ...
    nbotrim = nbo_utils.nbo_trimdiff_nbo7(nbo_headerE2_1,nbo_intE2_1,nbo_diff1,nbo_F1,nbo_headerE4_1,nbo_intSE4_1,nbo_intE4_1,
                                          nbo_headerE2_2,nbo_intE2_2,nbo_diff2,nbo_F2,nbo_headerE4_2,nbo_intSE4_2,nbo_intE4_2, verbose=verbose)
    nbo_headerE2_1_t = nbotrim[0]
    nbo_intE2_1_t = nbotrim[1]
    nbo_headerE4_1_t = nbotrim[4]
    nbo_intE4_1_t = nbotrim[6]
    nbo_headerE2_2_t = nbotrim[7]
    nbo_intE2_2_t = nbotrim[8]
    nbo_headerE4_2_t = nbotrim[11]
    nbo_intE4_2_t = nbotrim[13]
    
    if verbose==True:
        print("td1",len(nbo_intE4_1_t),len(nbo_intE4_1_t[0]))
        print("td2",len(nbo_intE4_2_t),len(nbo_intE4_2_t[0]))
        print("td1",len(nbo_headerE4_1_t),len(nbo_headerE4_1_t[0]))
        print("td2",len(nbo_headerE4_2_t),len(nbo_headerE4_2_t[0]))
    
    for i in range(len(nbo_headerE2_1_t)):
        for j in range(len(nbo_headerE2_1_t[i])):
            for x in nbo_headerE2_1_t[i][j]:#[k]:
                if x == "/" :
                    rep=nbo_headerE2_1_t[i][j].split("/") 
                    nbo_headerE2_1_t[i][j] = rep[0] + "\n" + rep[1]
    for i in range(len(nbo_headerE4_1_t)):
        for j in range(len(nbo_headerE4_1_t[i])):
            for x in nbo_headerE4_1_t[i][j]:
                if x == "/" :
                    rep=nbo_headerE4_1_t[i][j].split("/") 
                    nbo_headerE4_1_t[i][j] = rep[0] + "\n" + rep[1]
    ...
    if verbose==True:
        ...
        print("length of nbo_intE2_1", len(nbo_intE2_1))
        print("length of nbo_intE4_1", len(nbo_intE4_1))
    ...
    nbo_intE2_1_fit = reg.fit(nbo_intE2_1_t, crit_list1_0N, maxlong, cc_a1_renorm, verbose=verbose)[0]
    nbo_intE4_1_fit = reg.fit(nbo_intE4_1_t, crit_list1_0N, maxlong, cc_a1_renorm, verbose=verbose)[0]
    nbo_intE2_2_fit = reg.fit(nbo_intE2_2_t, crit_list2_0N, maxlong, cc_a2_renorm, verbose=verbose)[0]
    nbo_intE4_2_fit = reg.fit(nbo_intE4_2_t, crit_list2_0N, maxlong, cc_a2_renorm, verbose=verbose)[0]
    
    
    if FIT == True:
        print("======================== REG ANALYSIS =======================")
    
        reg_nboE2_1_fit = reg.regnbo(total_energy_wfx1_fit, cc_a_sampled, nbo_intE2_1_fit,
                                 np=POINTS, critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list_fit, verbose=verbose)
        reg_nboE4_1_fit = reg.regnbo(total_energy_wfx1_fit, cc_a_sampled, nbo_intE4_1_fit,
                                 np=POINTS, critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list_fit, verbose=verbose)
        reg_nboE2_2_fit = reg.regnbo(total_energy_wfx2_fit, cc_a_sampled, nbo_intE2_2_fit,
                                 np=POINTS, critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list_fit, verbose=verbose)
        reg_nboE4_2_fit = reg.regnbo(total_energy_wfx2_fit, cc_a_sampled, nbo_intE4_2_fit,
                                 np=POINTS, critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list_fit, verbose=verbose)
        
        reg_trimE2_1_fit    = reg.reg_zeroes(reg_nboE2_1_fit, [nbo_headerE2_1, nbo_intE2_1_fit, nbo_diff1, nbo_F1], verbose=verbose)
        reg_nboE2_1_fit     = reg_trimE2_1_fit[0]
        nbo_headerE2_1      = reg_trimE2_1_fit[1][0]
        nbo_intE2_1         = reg_trimE2_1_fit[1][1]
        nbo_diff1           = reg_trimE2_1_fit[1][2]
        nbo_F1              = reg_trimE2_1_fit[1][3]
        
        reg_trimE4_1_fit    = reg.reg_zeroes(reg_nboE4_1_fit, [nbo_headerE4_1, nbo_intE4_1_fit, nbo_intSE4_1], verbose=verbose)
        reg_nboE4_1_fit     = reg_trimE4_1_fit[0]
        nbo_headerE4_1      = reg_trimE4_1_fit[1][0]
        nbo_intE4_1         = reg_trimE4_1_fit[1][1]
        nbo_intSE4_1        = reg_trimE2_1_fit[1][2]
        
        reg_trimE2_2_fit    = reg.reg_zeroes(reg_nboE2_2_fit, [nbo_headerE2_2, nbo_intE2_2_fit, nbo_diff2, nbo_F2], verbose=verbose)
        reg_nboE2_2_fit     = reg_trimE2_2_fit[0]
        nbo_headerE2_2      = reg_trimE2_2_fit[1][0]
        nbo_intE2_2         = reg_trimE2_2_fit[1][1]
        nbo_diff2           = reg_trimE2_2_fit[1][2]
        nbo_F2              = reg_trimE2_2_fit[1][3]
        
        reg_trimE4_2_fit    = reg.reg_zeroes(reg_nboE4_2_fit, [nbo_headerE4_2, nbo_intE4_2_fit, nbo_intSE4_2], verbose=verbose)
        reg_nboE4_2_fit     = reg_trimE4_2_fit[0]
        nbo_headerE4_2      = reg_trimE4_2_fit[1][0]
        nbo_intE4_2         = reg_trimE4_2_fit[1][1]
        nbo_intSE4_2        = reg_trimE2_2_fit[1][2]
        
        """
        indice_removal=[]
        for i in range(len(reg_nboE2_1_fit[0])):
            for j in range(len(reg_nboE2_1_fit[0][i])):
                    if reg_nboE2_1_fit[0][i][j] == 0:
                        print("NBO_1",i,j,reg_nboE2_1_fit[0][i][j],nbo_intE2_1_fit[i][j],nbo_headerE2_1[i][j])
                        indice_removal.append([i,j])
        for i in range(len(indice_removal)):
            reg_nboE2_1_fit[0][indice_removal[i][0]].pop(indice_removal[i][1])
            reg_nboE2_1_fit[1][indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_headerE2_1[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_intE2_1_fit[indice_removal[i][0]].pop(indice_removal[i][1])
        indice_removal=[]
        for i in range(len(reg_nboE4_1[0])):
            for j in range(len(reg_nboE4_1[0][i])):
                    if reg_nboE4_1[0][i][j] == 0:
                        print("NBO_1",i,j,reg_nboE4_1[0][i][j],nbo_intE4_1[i][j],nbo_headerE4_1[i][j])
                        indice_removal.append([i,j])
        for i in range(len(indice_removal)):
            reg_nboE4_1_fit[0][indice_removal[i][0]].pop(indice_removal[i][1])
            reg_nboE4_1_fit[1][indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_headerE4_1[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_intE4_1_fit[indice_removal[i][0]].pop(indice_removal[i][1])
        indice_removal=[]
        for i in range(len(reg_nboE2_2[0])):
            for j in range(len(reg_nboE2_2[0][i])):
                    if reg_nboE2_2[0][i][j] == 0:
                        print("NBO_2",i,j,reg_nboE2_2[0][i][j],nbo_intE2_2[i][j],nbo_headerE2_2[i][j])
                        indice_removal.append([i,j])
        for i in range(len(indice_removal)):
            reg_nboE2_2_fit[0][indice_removal[i][0]].pop(indice_removal[i][1])
            reg_nboE2_2_fit[1][indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_headerE2_2[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_intE2_2_fit[indice_removal[i][0]].pop(indice_removal[i][1])
        indice_removal=[]
        for i in range(len(reg_nboE4_2_fit[0])):
            for j in range(len(reg_nboE4_2_fit[0][i])):
                    if reg_nboE4_1[0][i][j] == 0:
                        print("NBO_2",i,j,reg_nboE4_2_fit[0][i][j],nbo_intE4_2_fit[i][j],nbo_headerE4_2[i][j])
                        indice_removal.append([i,j])
        for i in range(len(indice_removal)):
            reg_nboE4_2_fit[0][indice_removal[i][0]].pop(indice_removal[i][1])
            reg_nboE4_2_fit[1][indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_headerE4_2[indice_removal[i][0]].pop(indice_removal[i][1])
            nbo_intE4_2_fit[indice_removal[i][0]].pop(indice_removal[i][1])
    ...
    nbo_intE2 = nbo_intE2_2_fit - nbo_intE2_1_fit
    nbo_intE4 = nbo_intE4_2_fit - nbo_intE4_1_fit
    ...
    #Read term signs
    sign_seg_E21=nbo_utils.sign_segm_nbo(nbo_intE2_1_fit, crit_list_fit, verbose=verbose)
    sign_seg_E41=nbo_utils.sign_segm_nbo(nbo_intE4_1_fit, crit_list_fit, verbose=verbose)   
    sign_seg_E22=nbo_utils.sign_segm_nbo(nbo_intE2_2_fit, crit_list_fit, verbose=verbose)
    sign_seg_E42=nbo_utils.sign_segm_nbo(nbo_intE4_2_fit, crit_list_fit, verbose=verbose)
    if verbose==True:
        print("Term signs for E4 terms, path 1",nbo_intE4_1_fit)
        print("Term signs for E4 terms, path 2",nbo_intE4_2_fit)
    sign_seg1 = [sign_seg_E21,sign_seg_E41]
    sign_seg2 = [sign_seg_E22,sign_seg_E42]
    ...
    if DIFF == True:
        reg_nboE2 = reg.regnbo(total_energy_wfx, cc_a_sampled, nbo_intE2, np=POINTS,
                            critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list_fit, verbose=verbose)
        reg_nboE4 = reg.regnbo(total_energy_wfx, cc_a_sampled, nbo_intE4, np=POINTS,
                            critical=AUTO_fit, inflex=INFLEX, critical_index=crit_list_fit, verbose=verbose)
        
        reg_trimE2 = reg.reg_zeroes(reg_nboE2, [nbo_headerE2_1_t, nbo_intE2], verbose=verbose)
        reg_nboE2 = reg_trimE2[0]
        if NOFIT == False:
            nbo_headerE2_1_t = reg_trimE2[1][0]
        nbo_intE2 = reg_trimE2[1][1]
        
        reg_trimE4 = reg.reg_zeroes(reg_nboE4, [nbo_headerE4_1_t, nbo_intE4], verbose=verbose)
        reg_nboE4 = reg_trimE4[0]
        if NOFIT == False:
            nbo_headerE4_1_t = reg_trimE4[1][0]
        nbo_intE4 = reg_trimE4[1][1]
    ...
    if (NOFIT == True or FIT == True or DIFF == True) and verbose==True:
        print("I",len(reg_nboE2_1[0]),len(nbo_headerE2_1[0]),len(reg_nboE4_1[0]),len(nbo_headerE4_1[0]))
    ...
    if WRITE == True:
        if NOFIT == True:
            if len(crit_list1) == 2:
                ...
                g = False
            dataframe_list1=reg.csvnbo7(reg_nboE2_1, nbo_headerE2_1, reg_nboE4_1, nbo_headerE4_1, SYS1, path1, g, sign_seg1, verbose=verbose)
            dataframe_list2=reg.csvnbo7(reg_nboE2_2, nbo_headerE2_2, reg_nboE4_2, nbo_headerE4_2, SYS2, path2, g, sign_seg2, verbose=verbose)
            #reg.excelnbo7(reg_nboE2_1,nbo_headerE2_1,reg_nboE4_1,nbo_headerE4_1,SYS1,path1,False)
            #reg.excelnbo7(reg_nboE2_2,nbo_headerE2_2,reg_nboE4_2,nbo_headerE4_2,SYS2,path2,False)
        if FIT == True:
            dataframe_list1_fit=reg.csvnbo7(reg_nboE2_1_fit, nbo_headerE2_1_t, reg_nboE4_1_fit, nbo_headerE4_1_t, SYS, path, False, sign_seg1, verbose=verbose)
            dataframe_list2_fit=reg.csvnbo7(reg_nboE2_2_fit, nbo_headerE2_2_t, reg_nboE4_2_fit, nbo_headerE4_2_t, SYS, path, False, sign_seg2, verbose=verbose)
            #reg.excelnbo7(reg_nboE2_1_fit,nbo_headerE2_1_t,reg_nboE4,nbo_headerE4_1_t,SYS,path,False)
            #reg.excelnbo7(reg_nboE2_2_fit,nbo_headerE2_2_t,reg_nboE4,nbo_headerE4_2_t,SYS,path,False)
        if DIFF == True:
            if verbose==True:
                print("Lengths of nbo_E2 (and its header):",len(reg_nboE2),len(nbo_headerE2_1_t))
                print("Lengths of nbo_E4 (and its header):",len(reg_nboE4),len(nbo_headerE4_1_t))
            dataframe_list=reg.csvnbo7(reg_nboE2, nbo_headerE2_1_t, reg_nboE4, nbo_headerE4_1_t, SYS, path, False, sign_seg1, verbose=verbose)
            #reg.excelnbo7(reg_nboE2,nbo_headerE2_1_t,reg_nboE4,nbo_headerE4_1_t,SYS,path,False)
        # bloup
    ...
    if DETAILED_ANALYSIS ==True:
             ...
             for i in range(len(dataframe_list1)): #AC change: R to R2. 
                ...
                if dataframe_list1[i]['REG'].dtype != np.dtype(object) :
                    if i == 0 and len(crit_list1) == 2 :
                        rv.generate_data_vis(dataframe_list1[i], [dataframe_list1[i]['R2'] for i in range(len(nbo_intE2_1))],
                                                    n_terms, save=SAVE_FIG, file_name=path1+'/'+SYS1+ '-NBO_detailed_seg_1+2_'+lang+'.png',
                                                    title = SYS1 +' NBO seg. 1+2', color = COLOR_TERMS, lang=lang, verbose=verbose) #_3seg
                    else:
                        rv.generate_data_vis(dataframe_list1[i], [dataframe_list1[i]['R2'] for i in range(len(nbo_intE2_1))],
                                                    n_terms, save=SAVE_FIG, file_name=path1+'/'+SYS1+ '-NBO_detailed_seg_'+ str(i+1) + '_' +lang +'.png',
                                                    title = SYS1 +' NBO seg. '+str(i+1), color = COLOR_TERMS, lang=lang, verbose=verbose) #_3seg
             for i in range(len(dataframe_list2)): #AC change: R to R2.
                 ...
                 if dataframe_list2[i]['REG'].dtype != np.dtype(object):
                     if i == 0 and len(crit_list2) == 2 :
                         rv.generate_data_vis(dataframe_list2[i], [dataframe_list2[i]['R2'] for i in range(len(nbo_intE2_2))],
                                                    n_terms, save=SAVE_FIG, file_name=path1+'/'+SYS1+ '-NBO_detailed_seg_1+2_'+lang+'.png',
                                                    title = SYS1 +' NBO seg. 1+2', color = COLOR_TERMS, lang=lang, verbose=verbose) #_3seg
                     else:
                         rv.generate_data_vis(dataframe_list2[i], [dataframe_list2[i]['R2'] for i in range(len(nbo_intE2_2))],
                                                    n_terms, save=SAVE_FIG, file_name=path2+'/'+SYS2+ '-NBO_detailed_seg_'+ str(i+1) + '_' +lang +'.png',
                                                    title = SYS2 +' NBO seg. '+str(i+1), color = COLOR_TERMS, lang=lang, verbose=verbose) #_3seg
            
               
        if FIT == True:
            for i in range(len(dataframe_list1_fit)): #AC change: R to R2.
                if i == 0 and len(crit_list1) == 2 :
                    rv.generate_data_vis(dataframe_list1_fit[i], [dataframe_list1_fit[i]['R2'] for i in range(len(nbo_intE2_1_fit))],
                                                    n_terms, save=SAVE_FIG, file_name=path1+'/'+ SYS_f1+'-NBO_detailed_seg_1+2_'+lang+'.png',
                                                    title = SYS_f1 +' NBO seg. 1+2', size_title=SIZE_TITLE, size_label=SIZE_LABEL, 
                                                    color = COLOR_TERMS, lang=lang, verbose=verbose)
                else:
                    rv.generate_data_vis(dataframe_list1_fit[i], [dataframe_list1_fit[i]['R2'] for i in range(len(nbo_intE2_1_fit))],
                                                    n_terms, save=SAVE_FIG, file_name=path1+'/'+ SYS_f1+'-NBO_detailed_seg_'+ str(i+1) + '_' +lang +'.png',
            ...
            for i in range(len(dataframe_list2_fit)): #AC change: R to R2.
                if i == 0 and len(crit_list1) == 2 :    
                    rv.generate_data_vis(dataframe_list2_fit[i], [dataframe_list2_fit[i]['R2'] for i in range(len(nbo_intE2_2_fit))],
                                                            n_terms, save=SAVE_FIG, file_name=path2+'/'+ SYS_f2+'-NBO_detailed_seg_1+2_'+lang+'.png',
                                                            title = SYS_f2 +' NBO seg. 1+2', size_title=SIZE_TITLE, size_label=SIZE_LABEL, 
                                                            color = COLOR_TERMS, lang=lang, verbose=verbose)
                else:
                    rv.generate_data_vis(dataframe_list2_fit[i], [dataframe_list2_fit[i]['R2'] for i in range(len(nbo_intE2_2_fit))],
                                                            n_terms, save=SAVE_FIG, file_name=path2+'/'+ SYS_f2+'-NBO_detailed_seg_'+ str(i+1) + '_' +lang +'.png',
        ...
        if DIFF == True:
            for i in range(len(dataframe_list)): #AC change: R to R2.
                if i == 0 and len(crit_list1) == 2 :
                    rv.generate_data_vis(dataframe_list[i], [dataframe_list[i]['R2'] for i in range(len(nbo_intE2))],
                                                    n_terms, save=SAVE_FIG, file_name=path+'/'+ SYS+'-NBO_detailed_seg_1+2_'+lang+'.png',
                                                    title = SYS +' NBO seg. 1+2', size_title=SIZE_TITLE, size_label=SIZE_LABEL, 
                                                    color = COLOR_TERMS, color_sign = COLOR_SIGN, sign_term = [sign_seg1[0][i],sign_seg1[1][i]], 
                                                    lang=lang, verbose=verbose)
                else:
                    rv.generate_data_vis(dataframe_list[i], [dataframe_list[i]['R2'] for i in range(len(nbo_intE2))],
                                                    n_terms, save=SAVE_FIG, file_name=path+'/'+ SYS+'-NBO_detailed_seg_'+ str(i+1) + '_' +lang +'.png',
                                                    title = SYS +' NBO seg. '+str(i+1), size_title=SIZE_TITLE, size_label=SIZE_LABEL,
                                                    color = COLOR_TERMS, color_sign = COLOR_SIGN, sign_term = [sign_seg1[0][i],sign_seg1[1][i]], 
                                                    lang=lang, verbose=verbose)

    print("================== REG-NBO V4 COMPLETED =================")
    #if verbose == True:
        #sys.stdout.close()    
