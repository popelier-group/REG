...
layout = [[sg.Checkbox('Auto Critical Point Search', key='-AUTO-'),sg.InputText("Enter Number of minimum points for search", key='-POINTS-')],
          [sg.Checkbox('Inflex',key='-INFLEX-'), sg.Checkbox('Write CSV Files',key='-WRITE-')],
          [sg.Checkbox('Save Figures', key='-SAVE_FIG-'),sg.Checkbox('Annotate Figures', key='-ANNOTATE-'), sg.Checkbox('Label Figures',key='-LABELS-')],
          [sg.Checkbox('Detailed Analysis', key='-DETAILED_ANALYSIS-'), sg.Text('N Terms Analysis'), sg.Input(size=(2,2), key='-N_TERMS-')],
          [sg.Button("Done")]
         ]
...
AUTO = values['-AUTO-']
if AUTO == False:
    quickLayout = [[sg.Text('Insert critical points positions separated by a comma')],
                   [sg.InputText()],
                   [sg.Button('Done')]
                   ]
...
iqa_inter_header_temp = np.array(aimall_utils.inter_property_from_int_file(folders,inter_prop, atoms)[1]) #used for reference
...
reg_intra = reg.reg(total_energy_wfn, cc, iqa_intra_temp, np=POINTS, critical=AUTO, inflex=INFLEX)
#INTER ATOMIC CONTRIBUTION
reg_inter = reg.reg(total_energy_wfn, cc, iqa_inter_temp, np=POINTS, critical=AUTO, inflex=INFLEX)
...
if WRITE == True:
    ...
    for i in range(len(reg_inter[0])):
        ...
        df_inter.to_csv(working_folder + "/Inter_seg_" + str(i + 1) + ".csv", sep=',')  # inter
        ...
        df_intra.to_csv(working_folder + "/Intra_seg_" + str(i + 1) + ".csv", sep=',')  # intra
...
rv.plot_segment(cc, 2625.50 * total_energy_wfn, critical_points, annotate=ANNOTATE, label=LABELS,
                y_label=r'Energy [$kJ.mol^{-1}$]', x_label=r'r (a.u)', title=working_folder.split('/')[-1],
                save=SAVE_FIG, file_name=working_folder + '/REG_analysis.png')

if DETAILED_ANALYSIS == True:
    for i in range(len(reg_inter[0])):
        rv.generate_data_vis(dataframe_list[i], [dataframe_list[i]['R'] for i in range(len(reg_inter[0]))],
                             n_terms, save=SAVE_FIG, file_name=working_folder + '/detailed_seg_' + str(i + 1) + '.png',
